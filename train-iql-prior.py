...
def parse_args():
    parser = argparse.ArgumentParser("Reinforcement Learning experiments for cooperative multiagent environments")
    # Environment
    ...
    parser.add_argument("--exp-name", type=str, default='self-iql2-prior-v1', help="name of the experiment")
...
def main():
    ...
    replay_buffer = PrioritizedReplayBuffer(
        int(arglist.max_buffer_size), len(obs_shape_n), no_features, no_actions, alpha)  # Init Buffer
    beta_schedule = linear_schedule.LinearSchedule(max_step, 1.0, initial_beta)
    ...
    while True:
        ...
        if train_step >= batch_size * arglist.max_episode_len and train_step % arglist.update_rate == 0:
            ...
            actions = np.swapaxes(actions, 1, 0)
            with tf.GradientTape() as tape:
                ...
                if "huber" in arglist.loss_type:
                    huber_loss = u.huber_loss(pred, tf.stop_gradient(y))
                    loss = tf.reduce_sum(huber_loss)
                elif "mse" in arglist.loss_type:
                    # TODO: It will have an error with prioritized experience replay
            ...
            replay_buffer.update_priorities(indices, huber_loss + prioritized_replay_eps)
        ...
        # update target model every step
