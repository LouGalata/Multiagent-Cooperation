...
class MADDPGAgent(AbstractAgent):
    def __init__(self, obs_space_n, act_space_n, agent_index, batch_size, buff_size, lr, num_units, gamma,
        ...
        self.policy = MADDPGPolicyNetwork(num_units, lr, obs_shape_n, act_shape_n[agent_index], act_type, 1,
                                          self.critic, agent_index)
        self.policy_target = MADDPGPolicyNetwork(num_units, lr, obs_shape_n, act_shape_n[agent_index], act_type, 1,
                                                 self.critic, agent_index)
...
class MADDPGPolicyNetwork(object):
    def __init__(self, units_per_layer, lr, obs_n_shape, act_shape, act_type,
        ...
        init_layer = tf.keras.layers.Dense(units_per_layer, activation='relu')(self.obs_input)
        med_layer = tf.keras.layers.Dense(units_per_layer, activation='relu')(init_layer)


        if self.use_gumbel:
            self.output_layer = tf.keras.layers.Dense(self.act_shape, activation='linear')(med_layer)
        else:
            self.output_layer = tf.keras.layers.Dense(self.act_shape, activation='tanh')(med_layer)

        self.model = tf.keras.Model(inputs=[self.obs_input], outputs=[self.output_layer])
    ...
    def forward_pass(self, obs):
        ...
        """
        return self.model(obs)  # log probabilities of the gumbel softmax dist are the output of the network
...
class MADDPGCriticNetwork(object):
    def __init__(self, units_per_layer, lr, obs_n_shape, act_shape_n, act_type, agent_index):
        """
        ...
        self.input_concat_layer = tf.keras.layers.concatenate(self.act_input_n)

        init_layer = tf.keras.layers.Dense(units_per_layer, activation='relu')(self.input_concat_layer)
        med_layer = tf.keras.layers.Dense(units_per_layer, activation='relu')(init_layer)

        self.output_layer = tf.keras.layers.Dense(1, activation='linear')(med_layer)

        # connect layers
        self.model = tf.keras.Model(inputs=self.obs_input_n + self.act_input_n, outputs=[self.output_layer])
    ...
    def _train_step_internal(self, concatenated_input, target_q, weights):
        ...
        """
        with tf.GradientTape() as tape:
            q_pred = self.model(concatenated_input)
