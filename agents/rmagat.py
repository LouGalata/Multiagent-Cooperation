...
class MAGATAgent(AbstractAgent):
    ...
    def update(self, agents, step):
        ...
        adjacency = [self.get_adj(a, self.k_lst, self.no_agents) for a in
                     np.swapaxes(obs, 1, 0)]
        adjacency = np.array(adjacency)  # shape: (batch_size, no_agents, no_agents)

        target_q_next = self.critic_target.predict(next_obs, target_act_next, adjacency)
        q_train_target = rew_n[:, None] + self.decay * target_q_next

        td_loss = self.critic.train_step(obs, acts_n, adjacency, q_train_target).numpy()[:, 0]
...
class MADDPGPolicyNetwork(object):
    ...
    def train(self, obs_n, act_n, adjacency):
        with tf.GradientTape() as tape:
            ...
            obs = [a[:, -1] for a in obs_n]  # Keep only tha last observation
            concatenated_input = tf.concat([obs, act_n], axis=-1)
